...
<head>
    ...
    <title>Tambah Produk Baru | CV. Srikandi Keris Barokah</title>
...
<body>
        ...
        <div class="form-card">
            <div class="form-header">
                ...
                <div>
                    ...
                    <p style="color:var(--color-text-muted); font-size:13px;">Silakan isi formulir di bawah ini untuk menambahkan produk atau layanan baru.</p>
                </div>
            </div>
            ...
            <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    <div class="form-group full-width">
                        ...
                        <input type="text" name="name" id="name" class="form-control" placeholder="Contoh: Serum Wajah Herbal Srikandi" value="{{ old('name') }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="category" id="category" class="form-control" required>
                            ...
                            <option value="kesehatan" {{ old('category') == 'kesehatan' ? 'selected' : '' }}>Kesehatan & Herbal</option>
                            <option value="terapi" {{ old('category') == 'terapi' ? 'selected' : '' }}>Alat Terapi</option>
                            ...
                            <option value="travel" {{ old('category') == 'travel' ? 'selected' : '' }}>Haji & Umroh</option>
                            <option value="properti" {{ old('category') == 'properti' ? 'selected' : '' }}>Properti</option>
                            <option value="wo" {{ old('category') == 'wo' ? 'selected' : '' }}>Wedding Organizer</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label for="price" class="form-label">Harga Tampilan</label>
                        <input type="text" name="price" id="price" class="form-control" placeholder="Contoh: Rp 85.000 atau Rp 2 Jt (DP)" value="{{ old('price') }}" required>
                    </div>

                    <div class="form-group">
                        <label for="badge" class="form-label">Label (Badge)</label>
                        <input type="text" name="badge" id="badge" class="form-control" placeholder="Contoh: Terlaris, Premium, BPOM (Opsional)" value="{{ old('badge') }}">
                    </div>

                    <div class="form-group">
                        <label for="rating" class="form-label">Rating</label>
                        <input type="number" name="rating" id="rating" class="form-control" min="1.0" max="5.0" step="0.1" placeholder="Contoh: 5.0" value="{{ old('rating', '5.0') }}" required>
                    </div>

                    <div class="form-group full-width">
                        <label for="description" class="form-label">Deskripsi Singkat</label>
                        <textarea name="description" id="description" class="form-control" rows="4" placeholder="Tuliskan deskripsi khasiat, kelayakan, atau detail produk..." required>{{ old('description') }}</textarea>
                    </div>

                    <div class="form-group">
                        <label for="image" class="form-label">Foto Produk (Utama)</label>
                        <input type="file" name="image" id="image" class="form-control">
                        <p class="file-hint">Format: jpg, png, jpeg (Maks. 2MB). Kosongkan jika ingin memakai FontAwesome Icon.</p>
                    </div>

                    <div class="form-group">
                        <label for="icon" class="form-label">FontAwesome Icon (Alternatif)</label>
                        <input type="text" name="icon" id="icon" class="form-control" placeholder="Contoh: fas fa-droplet" value="{{ old('icon') }}">
                        <p class="file-hint">Digunakan jika Foto Utama kosong. Contoh: <code>fas fa-droplet</code>, <code>fas fa-spa</code>, <code>fas fa-kaaba</code></p>
                    </div>

                    <div class="form-group full-width" style="border-top: 1px solid rgba(0,0,0,0.06); padding-top: 15px;">
                        <div class="checkbox-group">
                            <input type="checkbox" name="has_detail" id="has_detail" value="1" class="checkbox-input" {{ old('has_detail') ? 'checked' : '' }} onchange="toggleDetailField()">
                    ...
                    <div class="form-group full-width" id="detail_image_group" style="display: none;">
                        ...
                        <p class="file-hint">Brosur infografis berukuran besar yang muncul ketika tombol "Detail" diklik.</p>
                    </div>

                    <div class="form-group full-width" style="border-top: 1px solid rgba(0,0,0,0.06); padding-top: 15px;">
                        <label for="whatsapp_url" class="form-label">WhatsApp URL Kustom (Opsional)</label>
                        <input type="text" name="whatsapp_url" id="whatsapp_url" class="form-control" placeholder="Hubungi wa.me/..." value="{{ old('whatsapp_url') }}">
    ...
    <script>
        ...
        // Run on load to set initial state based on validation old() values
